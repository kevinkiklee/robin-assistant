...
import { createHelpers } from '../core/scripts/lib/migration-helpers.js';
...
test('addConfigField adds nested key with default', async () => {
  const root = repo();
  writeFileSync(join(root, 'user-data/robin.config.json'), JSON.stringify({ user: { name: 'T' } }));
  const helpers = createHelpers(root);
  await helpers.addConfigField('memory.maxItems', 1000);
  const cfg = JSON.parse(readFileSync(join(root, 'user-data/robin.config.json'), 'utf-8'));
