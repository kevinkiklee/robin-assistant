...
test('buildCaptureRow shape — episode_id resolved when known', () => {
  const row = buildCaptureRow(
    { id: 'capture:c1', body: 'hello', kind: 'fact', origin: 'user', source: 'cli',
      ts: '2026-01-01T00:00:00Z', meta: { x: 1 } },
...
test('buildCaptureRow without episode → episode_id key absent', () => {
  const row = buildCaptureRow(
    { id: 'capture:c2', body: 'orphan', kind: 'journal', origin: 'user', source: 'cli',
      ts: '2026-01-01T00:00:00Z' },
    { v2_episode_id: null }
  );
  // We omit the field entirely so SurrealDB stores NONE rather than coercing JS null.
  assert.ok(!('episode_id' in row) || row.episode_id === undefined,
...
test('buildCaptureRow preserves v1 archived_at into meta', () => {
  const row = buildCaptureRow(
    { id: 'capture:c3', body: 'old', kind: 'fact', origin: 'user', source: 'cli',
      ts: '2026-01-01T00:00:00Z', archived_at: '2026-04-01T00:00:00Z' },
    { v2_episode_id: null }
  );
