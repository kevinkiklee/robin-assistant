...
const HOOK = fileURLToPath(new URL('../core/scripts/pre-commit-hook.js', import.meta.url));
...
test('pre-commit-hook passes when only core/ is staged', () => {
  const root = gitInit();
  writeFileSync(join(root, '.gitignore'), '/user-data/\n');
  mkdirSync(join(root, 'core'));
  writeFileSync(join(root, 'core/foo.md'), 'x');
  execSync('git add core/foo.md', { cwd: root });
  const res = spawnSync('node', [HOOK], { cwd: root });
