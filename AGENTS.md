...
You are a personal systems co-pilot. This workspace is your persistent system. Read `user-data/ops/config/robin.config.json` for user name, timezone, email, assistant name.
...
- **Tamper detection.** Drift in `.claude/settings.json` hooks or in the loaded MCP server list is checked at session start by `system/scripts/diagnostics/check-manifest.js` against `user-data/ops/security/manifest.json`. Severe drift surfaces in the model context immediately; mild/info drift logs to `policy-refusals.log` (deduped 24h) for morning-briefing review. See `system/rules/security.md`.
- **Mechanical backstops.** PII patterns in writes to `user-data/memory/` block at the hook layer. High-stakes destination writes are audited to `user-data/ops/state/telemetry/high-stakes-writes.log`. AGENTS.md Hard Rules and `user-data/ops/jobs/` are integrity-checked at session start. Pattern TTL: 180 days inactivity → auto-archive via Dream. See `system/rules/security.md`.
...
1. Read `user-data/ops/state/sessions.md`. If it has rows with last-active <2h, note "Another session is active (platform X, started Y)" in your first response. **Append** your row to it (`<platform>-<timestamp>`) — NEVER overwrite the file (no `cat > sessions.md`, no `echo > sessions.md`; use `>>` append or read-modify-write only). Also drop rows with last-active >2h old.
2. Read `user-data/ops/state/jobs/failures.md`; mention any "Active failures" in your first response.
3. Read `user-data/ops/state/dream-state.md`. If `last_dream_at` is more than 28h before now, mention "Dream overdue (last ran <date>; <N> items in inbox)" in your first response and offer to run it inline (fetch `system/jobs/dream.md`, run Phase 2 inbox routing). Dream is `runtime: agent` and is the only writer that routes inbox entries to topic files; without periodic runs, captured facts never reach their destination.
4. Read `user-data/ops/config/integrations.md`, then read these files **in this exact order** (matters for prompt-cache reuse — frozen → slow → volatile): `user-data/memory/INDEX.md`, `user-data/memory/ENTITIES.md` (auto-generated entity index for fast recall; created if missing during first install), `user-data/memory/profile/identity.md`, `user-data/memory/profile/personality.md`, `user-data/memory/self-improvement/communication-style.md`, `user-data/memory/self-improvement/domain-confidence.md`, `user-data/memory/hot.md`, `user-data/memory/self-improvement/session-handoff.md`, `user-data/memory/self-improvement/learning-queue.md`. Open everything else on demand.
5. Scan `user-data/ops/jobs/` and `system/jobs/`. Same name → user-data wins (full) or merges (`override:` frontmatter). Read `custom-rules.md` if present.
...
| Job state / failures | `user-data/ops/state/jobs/INDEX.md` / `user-data/ops/state/jobs/failures.md` |
...
`user-data/ops/state/jobs/INDEX.md` for overview, `failures.md` for problems, `<name>.json` for detail. Don't spawn `robin jobs ...` — subprocess cost for data already on disk.
