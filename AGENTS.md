...
- **Tamper detection.** Drift in `.claude/settings.json` hooks or in the loaded MCP server list is checked at session start by `system/scripts/diagnostics/check-manifest.js` against `user-data/security/manifest.json`. Severe drift surfaces in the model context immediately; mild/info drift logs to `policy-refusals.log` (deduped 24h) for morning-briefing review. See `system/rules/security.md`.
