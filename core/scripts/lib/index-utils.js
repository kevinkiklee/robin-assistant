...
export function parseReferenceEntries(content) {
  ...
  while (i < lines.length) {
    ...
    if (/^- /.test(line)) {
      ...
      let entity = null;
      if (boldMatch) {
        // Normalize: strip trailing colon, lowercase, replace spaces with hyphens, strip remaining non-alphanumeric (except hyphens)
        entity = boldMatch[1]
          .replace(/:$/, '')
          .trim()
          .toLowerCase()
...
export function parseTaskEntries(content) {
  ...
  while (i < lines.length) {
    ...
    if (/^- \[[ x]\]/.test(line)) {
      ...
      entries.push({ text: taskLines.join('\n'), section: currentSection, lineIndex: startLineIndex });
      continue;
    }
  ...
  if (level === 'fact') {
    ...
    for (const [section, sectionEntries] of Object.entries(sections)) {
      ...
      for (const entry of sectionEntries) {
        ...
        lines.push(`  entity: ${entry.entity || ''}`);
        lines.push('  domains: []');
        lines.push('  related: []');
...
export function generateManifest(files) {
  ...
  for (const file of files) {
    ...
    lines.push(`  index: ${file.indexPath}`);
    lines.push(`  type: ${file.type}`);
    lines.push(`  entries: ${file.entries}`);
    const domains = file.domains && file.domains.length > 0 ? file.domains.join(', ') : '';
    lines.push(`  domains: [${domains}]`);
    const sections = file.sections && file.sections.length > 0 ? file.sections.join(', ') : '';
