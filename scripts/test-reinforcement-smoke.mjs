...
const { evaluatePending, REINFORCE_WINDOW_MS } = await import(`${ROOT}/src/recall/reinforcement.js`);
...
let [r1] = await db.query(`SELECT signal_count FROM ${m.id}`).collect();
...
let [r2] = await db.query(`SELECT signal_count, decay_anchor FROM ${m.id}`).collect();
...
console.log('recall_log outcomes:', logs.map((l) => l.outcome));
...
let [r3] = await db.query(`SELECT signal_count FROM ${m.id}`).collect();
