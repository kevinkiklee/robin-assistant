...
Grep audit:
```sh
grep -rE "process\.env\.[A-Z_]+(TOKEN|KEY|SECRET|PAT|PASS|REFRESH)" system/ user-data/scripts/
```
...
- `s6-supply-chain-postinstall.test.js` — synthetic Bash `cat user-data/secrets/.env` → blocked.
