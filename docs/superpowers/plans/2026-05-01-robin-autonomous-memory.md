...
- `user-data/runtime/state/turn/turn.json`, `turn-writes.log`, `capture-retry.json`, `capture-enforcement.log`, `recall.log`, `hook-perf.log`, `entities-hash.txt`
- `user-data/runtime/security/manifest.json` (re-snapshotted)
...
describe('turn-state', () => {
  ...
  it('readTurnJson returns null when corrupt', () => {
    const ws = setup();
    writeFileSync(join(ws, 'user-data/runtime/state/turn/turn.json'), '{not-json');
    assert.equal(readTurnJson(ws), null);
  });
  ...
  it('pruneWriteIntents drops entries older than cutoff', () => {
    const ws = setup();
    const now = Date.now();
    writeFileSync(join(ws, 'user-data/runtime/state/telemetry/turn-writes.log'),
    ...
    const text = readFileSync(join(ws, 'user-data/runtime/state/telemetry/turn-writes.log'), 'utf8');
...
const TURN_FILE = 'user-data/runtime/state/turn/turn.json';
const WRITES_LOG = 'user-data/runtime/state/telemetry/turn-writes.log';
const RETRY_FILE = 'user-data/runtime/state/turn/capture-retry.json';
...
function makeWs() {
  ...
  writeFileSync(join(ws, 'user-data/runtime/state/turn/turn.json'),
...
describe('PreToolUse write-intent logging', () => {
  it('appends to turn-writes.log when Edit targets user-data/memory/', () => {
    ...
    const event = JSON.stringify({
      tool_name: 'Edit',
      tool_input: { file_path: join(ws, 'user-data/memory/streams/inbox.md'), new_string: 'hello' },
    });
    const r = runHook(ws, ['--on-pre-tool-use'], event);
    assert.equal(r.exit ?? r.status, 0, r.stderr);
    const log = readFileSync(join(ws, 'user-data/runtime/state/telemetry/turn-writes.log'), 'utf8');
    assert.match(log, /\tt1\t/);
  ...
  it('does NOT log writes outside user-data/memory/', () => {
    ...
    const event = JSON.stringify({
      tool_name: 'Edit',
      tool_input: { file_path: join(ws, 'user-data/runtime/state/something.md'), new_string: 'x' },
    });
    runHook(ws, ['--on-pre-tool-use'], event);
    assert.equal(existsSync(join(ws, 'user-data/runtime/state/telemetry/turn-writes.log')), false);
  });
  ...
  it('logs Bash redirections to user-data/memory/', () => {
    ...
    const event = JSON.stringify({
      tool_name: 'Bash',
      tool_input: { command: 'echo "[fact] x" >> user-data/memory/streams/inbox.md' },
    });
  ...
  it('still blocks PII writes (existing behavior unchanged)', () => {
    ...
    const event = JSON.stringify({
      tool_name: 'Write',
      tool_input: {
        file_path: join(ws, 'user-data/memory/streams/inbox.md'),
...
describe('UserPromptSubmit handler', () => {
  it('writes turn.json with computed tier on substantive message', () => {
    ...
    mkdirSync(join(ws, 'user-data/state'), { recursive: true });
    mkdirSync(join(ws, 'user-data/memory'), { recursive: true });
    mkdirSync(join(ws, 'user-data/runtime/state/sessions.md').replace(/\/[^/]+$/, ''), { recursive: true });
    ...
    assert.equal(r.status, 0, r.stderr);
    const turn = JSON.parse(readFileSync(join(ws, 'user-data/runtime/state/turn/turn.json'), 'utf8'));
...
describe('Stop verifyCapture', () => {
  ...
  it('passes (exit 0) when tier 3 + write-intent recorded', () => {
    const ws = setupTier3WithoutCapture();
    writeFileSync(join(ws, 'user-data/runtime/state/telemetry/turn-writes.log'),
      `${new Date().toISOString()}\tclaude-code-x:111\tuser-data/memory/streams/inbox.md\tEdit\n`);
    const event = JSON.stringify({ session_id: 'claude-code-x' });
    const r = runHook(ws, ['--on-stop'], event);
    assert.equal(r.status, 0, r.stderr);
  });
  ...
  it('passes (exit 0) after retry budget exhausted', () => {
    const ws = setupTier3WithoutCapture();
    writeFileSync(join(ws, 'user-data/runtime/state/turn/capture-retry.json'),
  ...
  it('appends a telemetry line per outcome', () => {
    ...
    const log = readFileSync(join(ws, 'user-data/runtime/state/telemetry/capture-enforcement.log'), 'utf8');
...
function captureEnforcementEnabled(ws) {
  ...
  try {
    const cfg = JSON.parse(readFileSync(join(ws, 'user-data/runtime/config/robin.config.json'), 'utf8'));
...
function appendEnforcementLog(ws, line) {
  try {
    const file = join(ws, 'user-data/runtime/state/telemetry/capture-enforcement.log');
    mkdirSync(dirname(file), { recursive: true });
...
- Modify: `user-data/runtime/security/manifest.json` (Kevin's local instance — not in package)
...
```
Expected: writes updated `user-data/runtime/security/manifest.json` including the new `UserPromptSubmit` hook.
...
//
// Adds memory.capture_enforcement block to existing user-data/runtime/config/robin.config.json
...
export async function up(workspaceDir) {
  const file = join(workspaceDir, 'user-data/runtime/config/robin.config.json');
...
Run:
```sh
cp user-data/runtime/config/robin.config.json /tmp/robin.config.json.bak
node -e "import('./system/migrations/0009-capture-enforcement-config.js').then(m => m.up(process.cwd()).then(console.log))"
diff user-data/runtime/config/robin.config.json /tmp/robin.config.json.bak || echo 'config updated as expected'
```

Expected: shows the new `capture_enforcement` block added.

Restore: `cp /tmp/robin.config.json.bak user-data/runtime/config/robin.config.json` (we'll re-run migration via the proper flow during install).
...
```
Compare the hash to the current `user-data/runtime/security/manifest.json` value:
```sh
grep hardRulesHash user-data/runtime/security/manifest.json
```
...
Capture is enforced at end-of-turn by `system/scripts/hooks/claude-code.js --on-stop`. The hook checks whether `user-data/memory/` was written during the turn (via `user-data/runtime/state/telemetry/turn-writes.log`). If not, the model must declare a waiver inline:
...
If neither a write nor a marker is present and retry budget allows, the hook exits 2 with a corrective stderr message; the model receives the message and re-emits with capture or marker. Default `retry_budget: 1` (configurable in `user-data/runtime/config/robin.config.json` → `memory.capture_enforcement`).
...
```markdown
11.5. **Capture + recall telemetry review.** Read entries from `user-data/runtime/state/telemetry/capture-enforcement.log` and `user-data/runtime/state/recall.log` since `last_dream_at`. Surface in escalation report:
...
17.7. **Telemetry log rotation.** Cap each file to its limit:
   - `user-data/runtime/state/telemetry/capture-enforcement.log` → 5000 lines
...
`system/tests/fixtures/mock-hook-events/pre-tool-use-write.json`:
```json
{"tool_name":"Write","tool_input":{"file_path":"user-data/memory/streams/inbox.md","content":"- [fact] sample inbox entry"}}
```

`system/tests/fixtures/mock-hook-events/pre-tool-use-bash.json`:
```json
{"tool_name":"Bash","tool_input":{"command":"echo \"[fact] direct-bash entry\" >> user-data/memory/streams/inbox.md"}}
```
...
describe('golden-session-capture (E2E)', () => {
  it('full 4-turn flow: trivial → substantive-with-capture → substantive-without → recovery', () => {
    ...
    const turn2 = JSON.parse(readFileSync(join(ws, 'user-data/runtime/state/turn/turn.json'), 'utf8'));
    r = runHook(ws, ['--on-pre-tool-use'], JSON.stringify({
      tool_name: 'Edit',
      tool_input: { file_path: join(ws, 'user-data/memory/streams/inbox.md'), new_string: '- [task|origin=user] confirm dentist address' },
    }));
    assert.equal(r.status, 0);
    r = runHook(ws, ['--on-stop'], JSON.stringify({ session_id: 'claude-code-golden' }));
    assert.equal(r.status, 0);
    ...
    const turn3 = JSON.parse(readFileSync(join(ws, 'user-data/runtime/state/turn/turn.json'), 'utf8'));
    const tx = join(ws, 'transcript.jsonl');
    ...
    const log = readFileSync(join(ws, 'user-data/runtime/state/telemetry/capture-enforcement.log'), 'utf8').trim().split('\n');
...
- `user-data/runtime/state/telemetry/capture-enforcement.log` (out of scope for token measurement, skip)
...
node -e "import('./system/migrations/0009-capture-enforcement-config.js').then(m => m.up(process.cwd()).then(console.log))"
```
Expected: `{ changed: true }`. Verify `user-data/runtime/config/robin.config.json` now contains the `memory.capture_enforcement` block.
...
node system/scripts/diagnostics/manifest-snapshot.js --apply --confirm-trust-current-state
```
Expected: writes the updated `user-data/runtime/security/manifest.json` covering the new UserPromptSubmit hook.
...
- No errors in `user-data/runtime/state/hook-errors.log` (if it exists).

Check telemetry:
```sh
tail user-data/runtime/state/telemetry/capture-enforcement.log
tail user-data/runtime/state/recall.log
```
