...
function envPath(workspaceDir) {
  return join(workspaceDir, 'user-data/runtime/secrets/.env');
}
...
export function requireSecret(workspaceDir, key) {
  ...
  if (!value) {
    throw new Error(
      `Missing secret: ${key}. Add it to user-data/runtime/secrets/.env (see system/skeleton/secrets/README.md).`
    );
  }
  return value;
}
...
3. **Refactor pass** (single PR or commit chain):
   ...
   - **Grep audit** (mandatory before merge): `grep -rE "process\.env\.[A-Z_]+(TOKEN|KEY|SECRET|PAT|PASS|REFRESH)" system/ user-data/runtime/scripts/` returns zero matches outside `secrets.js`.
4. **Hook pass**:
...
5. **Refusal log rename** (if cycle-1b shipped first):
   - Rename file: `mv user-data/runtime/state/outbound-refusals.log user-data/runtime/state/telemetry/policy-refusals.log` (only if non-empty; otherwise let cycle-2a create fresh).
...
7. **Smoke**: synthetic Bash command `cat user-data/runtime/secrets/.env` blocked; subprocess `process.env` has no `DISCORD_BOT_TOKEN`.
...
| Risk | Mitigation |
|---|---|
| Refactor misses a `process.env.<SECRET>` reader | Mandatory grep audit in DoD #2; runs against `system/` and `user-data/runtime/scripts/`. |
...
| `loadSecrets` removal breaks an unknown caller | Removal pass with `grep -rn "loadSecrets" system/ user-data/runtime/scripts/`; replace any survivors. Tests would have caught a missing import. |
...
- `s6-supply-chain-postinstall.test.js` — synthetic postinstall script that attempts `cat user-data/runtime/secrets/.env` via Bash; the hook blocks at `secrets-read`. Pre-install file unchanged.
- `s8-jailbreak-cat-env.test.js` — synthetic `env | grep TOKEN` Bash command; hook blocks at `env-dump`.
- `s8-jailbreak-cat-secrets.test.js` — synthetic `cat user-data/runtime/secrets/.env` Bash command; hook blocks at `secrets-read`.

### 10.3 Smoke (manual, post-deploy)

1. Drop synthetic Bash `cat user-data/runtime/secrets/.env`; confirm hook blocks; refusal log entry; agent's next reply mentions block.
