...
- Manifest is per-install (not shipped). Lives in `user-data/runtime/security/`. Skeleton ships in `system/skeleton/security/` and is copied on first install.
...
│                                                                 │
│  user-data/runtime/security/manifest.json     ← per-install trusted     │
│              │                          baseline (gitignored).  │
│              ▼                                                  │
│   system/scripts/                                               │
...
- **Per-install (live):** `user-data/runtime/security/manifest.json` (gitignored under `user-data/`).
...
- The live manifest at `user-data/runtime/security/manifest.json` is initialized from the skeleton.
...
The manifest is plain JSON. An attacker who can edit `.claude/settings.json` can also edit `user-data/runtime/security/manifest.json`. The detection layer is **git review of pull diffs**: if a fork commits a malicious hook AND its corresponding manifest update, both diffs appear in the pull. Not a defense against an attacker already on the filesystem; documented in `system/rules/security.md` known limitations.
...
async function main() {
  ...
  if (!existsSync(manifestPath)) {
    process.stderr.write('WARNING: user-data/runtime/security/manifest.json missing; tamper detection inactive.\n');
...
diff user-data/runtime/security/manifest.json /tmp/snapshot.json
$EDITOR user-data/runtime/security/manifest.json   # paste in additions
```
...
Overwrites `user-data/runtime/security/manifest.json` with current state. The two-flag pattern (separate `--apply` + explicit `--confirm-trust-current-state`) prevents accidental "snapshot accepts whatever drift is in place." The flag name is intentionally long.
...
This overwrites user-data/runtime/security/manifest.json with current state, which
...
ls user-data/runtime/security/manifest.json     # exists; copied by setup.js (postinstall)
...
diff user-data/runtime/security/manifest.json /tmp/current-state.json
...
```markdown
- **Tamper detection.** Drift in `.claude/settings.json` hooks or in the loaded MCP server list is checked at session start by `system/scripts/diagnostics/check-manifest.js` against `user-data/runtime/security/manifest.json`. Severe drift surfaces in the model context immediately; mild/info goes to `policy-refusals.log` (deduped 24h) for morning-briefing review. See `system/rules/security.md`.
```
...
2. **Skeleton ship:** `system/skeleton/security/manifest.json` lands in the package; `setup.js` (postinstall) copies it to `user-data/runtime/security/manifest.json` if not already present.
...
2. `setup.js` (postinstall) copies skeleton → `user-data/runtime/security/manifest.json` if absent.
