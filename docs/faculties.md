...
- **Disable:** `robin hooks disable intuition` adds `intuition` to `hooks.disabled` (a `string[]`) in `<robinHome>/config.json`. Per-phase — disabling one leaves the others running.
