...
test('apply: writes hardware.yaml with detected profile + runtime block', () => {
  ...
  const path = writeHardwareYaml(dir, { cpu: 'Apple M5 Max', arch: 'arm64', ram_gb: 64, os: 'darwin-26.x', profile: 'm5-max-64gb' });
...
test('apply: chooses cloud-only profile for unknown hardware', () => {
  const dir = mkdtempSync(join(tmpdir(), 'robin-hw-'));
  const path = writeHardwareYaml(dir, { cpu: 'Intel Xeon', arch: 'x64', ram_gb: 8, os: 'linux-6', profile: 'cloud-only' });
  const doc = parseYaml(readFileSync(path, 'utf8'));
