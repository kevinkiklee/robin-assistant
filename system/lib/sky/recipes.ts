...
const fmtTime = (d: Date) =>
  d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }).toLowerCase().replace(' ', '');
...
const PRIORITY: Record<RecipeMatch['recipe'], number> = {
  sunset_color: 3, sunrise_color: 3, fog_sunrise: 2, moon: 2, rain_clearing: 1, tide_window: 1,
};
...
export function matchRecipes(input: {
  ...
  const { moon } = input;
  if (moon && moon.illumination >= SKY.moonMinIllumination && moon.horizonClear &&
  ...
  if (tide && inRange(tide.leadH, SKY.sunriseLeadHours)) {
    out.push({
      recipe: 'tide_window', window: 'sunrise', windowDate: input.dates.sunrise,
