...
test('new match fires a notification and records an alert', async () => {
  ...
  const m: RecipeMatch = {
    recipe: 'sunset_color',
    window: 'sunset',
    windowDate: '2026-06-25',
    title: '🌇 t',
    body: 'high cloud',
    key: 'sunset:2026-06-25',
    mergeGroup: 'sunset:2026-06-25',
  };
  const r = await fireMatches({
    db,
    matches: [m],
    openKeys: [],
    deliver: async (n) => {
...
test('two matches sharing a mergeGroup ⇒ exactly one notification, both keys in fired', async () => {
  ...
  const clearing: RecipeMatch = {
    recipe: 'rain_clearing',
    window: 'sunset',
    windowDate: '2026-06-25',
    title: '⛈️→☀️ t',
    body: 'storm breaking',
    key: 'clearing:2026-06-25',
    mergeGroup: 'sunset:2026-06-25',
  };
  const r = await fireMatches({
    db,
    matches: [color, clearing],
    openKeys: [],
    deliver: async (n) => {
      sent.push(n);
    },
