...
5. **Read context** — read `system/manifest.md` for a memory overview, then `user-data/profile.md` (personality and identity sections) and these sections of `user-data/self-improvement.md`: Session Handoff (continuity from prior session), Communication Style (how to interact), Domain Confidence (where to be cautious vs. autonomous), Learning Queue (one question ready if a natural moment arises). Load everything else on demand when the current task needs it. Don't summarize what you read unless asked.
