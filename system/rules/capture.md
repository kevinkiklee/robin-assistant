...
**When to capture (settled-class elision):** emit `[action]` for unsettled classes only — those that would change the agent's understanding of trust. Look up state in the compact summary in `user-data/runtime/config/policies.md`:
