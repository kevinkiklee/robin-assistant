...
import { actions, integration as github } from './index.ts';
...
test('github: tick ingests new notifications and dedupes by id', async () => {
  ...
  ctx.fetch = (async (_url: string, opts: { headers?: Record<string, string> }) => {
    ...
    assert.ok(auth);
    assert.match(auth, /Bearer fake-token/);
    if (callCount === 1) {
      return new Response(
        JSON.stringify([
          {
            id: 'n1',
            subject: { title: 'PR #42', type: 'PullRequest' },
            repository: { full_name: 'kevin/robin' },
            unread: true,
            reason: 'mention',
          ...
          {
            id: 'n2',
            subject: { title: 'Issue #7', type: 'Issue' },
            repository: { full_name: 'kevin/photo-tools' },
            unread: true,
            reason: 'assign',
    ...
    return new Response(
      JSON.stringify([
        ...
        {
          id: 'n3',
          subject: { title: 'PR #43', type: 'PullRequest' },
          repository: { full_name: 'kevin/robin' },
          unread: true,
...
test('github: actions.recent_activity resolves username and fetches events', async () => {
  ...
  ctx.fetch = (async (url: string) => {
    ...
    if (url.endsWith('/user'))
      return new Response(JSON.stringify({ login: 'kevin', name: 'Kevin' }), { status: 200 });
    return new Response(
      JSON.stringify([
        {
          id: 'e1',
          type: 'PushEvent',
          repo: { name: 'kevin/robin' },
          payload: {},
