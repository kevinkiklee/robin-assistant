...
import { integration as fin, actions } from './index.ts';
...
test('finance_quote: tick fetches default watchlist and ingests quotes', async () => {
  ...
  ctx.fetch = (async (url: string) => {
    if (url.includes('/v7/finance/quote')) {
      return new Response(JSON.stringify({
        quoteResponse: {
          result: [
            { symbol: 'SPY', regularMarketPrice: 500.12, regularMarketChange: 1.23, regularMarketChangePercent: 0.25, shortName: 'SPDR S&P 500' },
  ...
  const rows = db.prepare("SELECT body FROM events_content WHERE body LIKE 'SPY%'").all() as Array<{ body: string }>;
...
test('finance_quote: tick respects custom watchlist in state KV', async () => {
  ...
  ctx.fetch = (async (url: string) => {
    ...
    return new Response(JSON.stringify({ quoteResponse: { result: [{ symbol: 'AAPL', regularMarketPrice: 200 }, { symbol: 'MSFT', regularMarketPrice: 400 }] } }), { status: 200 });
  }) as typeof fetch;
  await fin.tick!(ctx);
  ...
  ctx.fetch = (async () => new Response(JSON.stringify({
    chart: {
      result: [{
        ...
        indicators: { quote: [{ open: [200, 201], high: [205, 203], low: [199, 200], close: [203, 202], volume: [1000, 2000] }] },
      }],
    },
