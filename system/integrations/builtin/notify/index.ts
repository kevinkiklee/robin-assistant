...
async function notifyMacOS(title: string, message: string): Promise<void> {
  ...
  await execFileP('osascript', ['-e', `display notification "${safeMsg}" with title "${safeTitle}"`]);
}

export const integration: Integration = {
  async health(_ctx: IntegrationContext) {
    return { ok: true, message: platform() === 'darwin' ? 'macOS osascript available' : 'limited (non-darwin)' };
  },
};

export async function notifyMacOSAction(params: { title: string; message: string }): Promise<{ delivered: boolean; reason?: string }> {
  try {
