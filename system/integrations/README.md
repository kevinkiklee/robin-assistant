...
The corresponding **scripts** live in `system/skeleton/scripts/` and are
copied to `user-data/scripts/` on first run:
...
2. Add credentials to `user-data/secrets/.env` (placeholders ship in
   `system/skeleton/secrets/.env.example`).
3. Run `node user-data/scripts/auth-<provider>.js` — opens a browser, runs
   the OAuth code flow against `127.0.0.1:<port>/oauth-callback`, writes
   the refresh token back to `.env`, caches the access token in
   `user-data/state/sync/<provider>.json`.
4. Bootstrap once: `node user-data/scripts/sync-<name>.js --bootstrap`.
...
live in `user-data/secrets/.env` (gitignored). Per-provider cursor state
