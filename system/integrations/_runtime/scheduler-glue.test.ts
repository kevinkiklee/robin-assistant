...
function makeIntegration(rootDir: string, name: string, schedule = '*/5 * * * *') {
  ...
  writeFileSync(join(dir, 'index.js'), `export const integration = { tick: async () => ({ status: 'ok' }) };\n`);
}
...
test('scheduler-glue: registers integrations and seeds cron jobs', async () => {
  ...
  const r = await registerIntegrations(fakeDaemon, db, () => null, { systemRoot: sysRoot, userDataRoot: userRoot });
...
test('scheduler-glue: skips event: schedules', async () => {
  ...
  const fakeDaemon = { registerHandler: () => {} } as unknown as Parameters<typeof registerIntegrations>[0];
  const r = await registerIntegrations(fakeDaemon, db, () => null, { systemRoot: sysRoot, userDataRoot: userRoot });
