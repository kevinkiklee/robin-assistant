...
// user-data/runtime/secrets/.env on demand. This prevents subprocess inheritance of
...
// Read a single secret from user-data/runtime/secrets/.env. Throws if missing.
...
export function requireSecret(workspaceDir, key) {
  ...
  if (!value) {
    throw new Error(
      `Missing secret: ${key}. Add it to user-data/runtime/secrets/.env (see system/scaffold/runtime/secrets/README.md).`
    );
  }
  return value;
}
