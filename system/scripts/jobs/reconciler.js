...
function readWorkspaceConfig(workspaceDir) {
  return readJSON(join(workspaceDir, 'user-data/runtime/config/robin.config.json'), {});
}
...
function reconcileInner({ workspaceDir, argv, force, adapter, tz, paths }) {
  ...
  // system/jobs or user-data/runtime/jobs (regardless of enabled state). Plists with
  // names outside this set share the LABEL_PREFIX namespace but are managed
  // by something else (e.g. user-data/runtime/scripts/discord-bot-install.js) — never
  // remove them.
