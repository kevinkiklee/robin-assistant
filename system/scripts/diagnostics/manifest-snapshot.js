...
//   → writes the snapshot to user-data/runtime/security/manifest.json.
...
function buildSnapshot(workspaceDir) {
  ...
  let userDataJobs = [];
  const jobsDir = join(workspaceDir, 'user-data/runtime/jobs');
...
async function main() {
  ...
  if (args.apply) {
    ...
    process.stderr.write('manifest-snapshot.js: wrote user-data/runtime/security/manifest.json\n');
    process.exit(0);
  }
