...
export async function migrateV2InDir(workspaceDir, pkgRoot) {
  ...
  const systemDir = join(pkgRoot, 'system');
  // v3 layout: state skeleton lives under system/skeleton/state/, AGENTS.md is at repo root.
  cpSync(join(systemDir, 'skeleton', 'state', 'sessions.md'), join(workspaceDir, 'state', 'sessions.md'));

  cpSync(join(pkgRoot, 'AGENTS.md'), join(workspaceDir, 'AGENTS.md'));
  for (const file of ['startup.md', 'capture-rules.md']) {
    cpSync(join(systemDir, file), join(workspaceDir, file));
  }
  ...
  cpSync(join(systemDir, 'operations'), protocolsDest, { recursive: true });
