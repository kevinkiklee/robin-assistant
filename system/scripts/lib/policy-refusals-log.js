// Policy refusals log — append-only TSV at user-data/state/policy-refusals.log.
//
...
//
// Rotation at 1MB: oldest log is moved to user-data/state/policy-refusals-YYYY-MM.log
...
const LOG_REL = 'user-data/state/policy-refusals.log';
...
function rotateIfLarge(workspaceDir) {
  ...
  const archive = join(workspaceDir, `user-data/state/policy-refusals-${monthTag}.log`);
