...
import { cleanupStaleLocks } from '../scripts/lib/jobs/lock-cleanup.js';

function makeWorkspace() {
  const root = mkdtempSync(join(tmpdir(), 'robin-lc-'));
  mkdirSync(join(root, 'user-data/state/jobs/locks'), { recursive: true });
  return root;
}

function writeLock(workspaceDir, name, content) {
  const p = join(workspaceDir, 'user-data/state/jobs/locks', name + '.lock');
...
describe('cleanupStaleLocks', () => {
  ...
  test('ignores non-.lock files in locks dir', () => {
    const root = makeWorkspace();
    writeFileSync(join(root, 'user-data/state/jobs/locks', 'README.txt'), 'ignored');
    const result = cleanupStaleLocks(root, { staleMs: 5 * 60 * 1000 });
    assert.deepEqual(result.removed, []);
    rmSync(root, { recursive: true, force: true });
  });
