...
Things Robin wants to understand better about the user. Lifecycle owned by Dream (`system/jobs/learning-queue.md`): population, daily selection + surfacing to `runtime/state/learning-queue/today.md`, in-session ask, closure via `[answer|qid=...]` capture markers, 60-day stale retire.

Each entry: `qid` (slug derived from heading), `domain`, `why`, `status` (`open` / `answered` / `dropped`), `added`. When closed, Dream appends `answered`, `answer`, `route`. When retired: `dropped`, `dropped_reason`.
