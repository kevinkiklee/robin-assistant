...
test('watchStatePath: returns correct path', () => {
  const ws = '/tmp/test-ws';
  assert.equal(watchStatePath(ws, 'my-watch'), '/tmp/test-ws/user-data/runtime/state/watches/my-watch.json');
});
...
test('writeWatchState + readWatchState: round-trip', () => {
  const ws = workspace();
  mkdirSync(join(ws, 'user-data/runtime/state/watches'), { recursive: true });
  const state = {
    id: 'round-trip',
...
test('writeWatchState: atomic write — no .tmp file left behind', () => {
  ...
  const tmpPath = join(ws, 'user-data/runtime/state/watches', 'atomic-test.json.tmp');
...
test('writeWatchState: creates state directory if missing', () => {
  ...
  const statePath = join(ws, 'user-data/runtime/state/watches/no-dir-test.json');
  assert.ok(existsSync(statePath), 'state file should be created');
});

test('readWatchState: handles corrupt JSON gracefully', () => {
  const ws = workspace();
  const dir = join(ws, 'user-data/runtime/state/watches');
  mkdirSync(dir, { recursive: true });
