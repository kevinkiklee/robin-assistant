...
test('creates all three files when absent', async () => {
  ...
  const policies = join(ws, 'user-data/policies.md');
  const trust = join(ws, 'user-data/memory/self-improvement/action-trust.md');
  const pending = join(ws, 'user-data/state/pending-asks.md');
...
test('idempotent: re-running on migrated state is a no-op', async () => {
  const ws = workspace();
  await up({ workspaceDir: ws });
  const before = readFileSync(join(ws, 'user-data/policies.md'), 'utf8');
  await up({ workspaceDir: ws });
  const after = readFileSync(join(ws, 'user-data/policies.md'), 'utf8');
...
test('creates parent directories if missing', async () => {
  ...
  assert.ok(existsSync(join(ws, 'user-data/state/pending-asks.md')));
});
