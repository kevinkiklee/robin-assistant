...
test('requireSecret throws when key missing in .env', () => {
  const ws = setup();
  writeFileSync(join(ws, 'user-data/runtime/secrets/.env'), 'OTHER=value\n');
...
test('requireSecret returns the value when present in .env', () => {
  const ws = setup();
  writeFileSync(join(ws, 'user-data/runtime/secrets/.env'), 'PRESENT_KEY=value\n');
...
test('saveSecret creates .env when missing and adds the key', () => {
  ...
  const content = readFileSync(join(ws, 'user-data/runtime/secrets/.env'), 'utf-8');
...
test('saveSecret replaces an existing key in place', () => {
  const ws = setup();
  writeFileSync(
    join(ws, 'user-data/runtime/secrets/.env'),
    '# leading comment\nFOO=old\nBAR=stable\n'
  );
...
test('saveSecret writes atomically (no .tmp left behind)', () => {
  const ws = setup();
  saveSecret(ws, 'K', 'v');
  assert.ok(!existsSync(join(ws, 'user-data/runtime/secrets/.env.tmp')));
  rmSync(ws, { recursive: true });
});
