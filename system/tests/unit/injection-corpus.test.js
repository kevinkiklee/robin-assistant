...
const corpus = JSON.parse(
  readFileSync(new URL('../fixtures/prompt-injection/corpus.json', import.meta.url), 'utf8'),
);
...
for (const entry of corpus) {
  test(`A: wrap survives ${entry.id} (${entry.technique})`, () => {
    const wrapped = wrapUntrusted(entry.body, {
      source: 'test',
      eventId: 'events:t',
      trust: 'untrusted',
    });
    ...
    assert.ok(
      !entry.body.includes(`</untrusted-content-${nonce}>`),
      `body cannot precompute nonce: ${entry.id}`,
    );
