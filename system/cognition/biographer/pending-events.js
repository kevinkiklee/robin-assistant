...
export async function listPendingEvents(db, { since, limit = 50 } = {}) {
  const q = since
    ? surql`SELECT id, ts, source FROM events WHERE biographed_at IS NONE AND ts >= ${since} ORDER BY ts ASC LIMIT ${limit}`
    : surql`SELECT id, ts, source FROM events WHERE biographed_at IS NONE ORDER BY ts ASC LIMIT ${limit}`;
