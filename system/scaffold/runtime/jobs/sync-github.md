...
enabled: false
schedule: "0 * * * *"
command: node user-data/runtime/scripts/sync-github.js
catch_up: true
timeout_minutes: 5
...
Disabled by default. Enable after running:
  ...
  # 2. Add GITHUB_PAT=<token> to user-data/runtime/secrets/.env
...
Auth uses GITHUB_PAT in user-data/runtime/secrets/.env (no OAuth flow).
