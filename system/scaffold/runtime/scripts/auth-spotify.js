...
// user-data/runtime/secrets/.env, set:
//   SPOTIFY_CLIENT_ID=...
...
async function main() {
  ...
  console.log('\n[auth-spotify] success — refresh token saved to user-data/runtime/secrets/.env');
  console.log('[auth-spotify] enable the sync job:');
  console.log('  node bin/robin.js jobs enable sync-spotify');
  console.log('  node user-data/runtime/scripts/sync-spotify.js --bootstrap');
}
