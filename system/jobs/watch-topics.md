...
1. Read `user-data/runtime/state/watches/<id>.json` for the fingerprint set.
...
- If `consecutive_failures >= 3`: set the watch's `enabled: false` and append a row to `user-data/runtime/state/jobs/failures.md` with category `watch_disabled` and reason.
